...
func get_all(startheight, latestheight uint64) (DepositMoments []cadence.Event, blockNumbers []uint64, timestamps []time.Time, err error) {
	...
	for startheight < latestheight {
		m, b, t, err :=get_momentdeposit(startheight, startheight+numQueriedBlocks,flowClient)
		if err != nil {
			if strings.Contains(err.Error(), "ResourceExhausted") {
				//log.Warn("resource exhausted, decrease number of queried blocks.")
				numQueriedBlocks /= 2
				/*add a condition to bypass numqueriedblocks = 0 situation*/
				if numQueriedBlocks < 1 {
                    fmt.Println(startheight)
                    startheight += 1
	...
	return //DepositMoments, blockNumbers, timestamps, err
}


func get_momentdeposit(startheight, endheight uint64, flowClient *client.Client) (DepositMoments []cadence.Event, blockNumbers []uint64, timestamps []time.Time, err error){
	...
	for _, blockEvent := range blockEvents {
		if len(blockEvent.Events) != 0 {
			for _, momentdepositdEvent := range blockEvent.Events {
			  DepositMoments = append(DepositMoments, momentdepositdEvent.Value)
...
func main() {
	...
	*/
	...
	/*var x14,y14,z14,w14 =*/ get_all(19050753, 21291691)   //stuck at 19054980, 1, 19252872
	/*
	fmt.Println(w14)

	file14, err := os.Create("myfile14test")
	if err != nil {
		log.Fatal(err)
	}
	...
	*/
	...
	for i := range x07 {
	fmt.Fprintln(mw07, x07[i].Fields[0], ",", x07[i].Fields[1], ",",  y07[i],",", z07[i])
	}
